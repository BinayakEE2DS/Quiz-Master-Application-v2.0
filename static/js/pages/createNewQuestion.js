const CreateNewQuestion = {
    template: `
      <div id="main">
        <div id="canvas">
          <div id="form-body">
            <h4 style="color: blue; text-align: center">New Question</h4><br>
    ...
    data() {
      return {
        ...
        quizId: this.$route.params.quiz_id, // Quiz ID from the route
      };
    },
    methods: {
      // Fetch the chapter name based on the quiz ID
      async fetchChapterName() {
        try {
          const response = await fetch(`/quiz_information/${this.quizId}`, {
            headers: {
      ...
      async saveQuestion() {
        ...
        try {
          const response = await fetch(`/create_new_question/${this.quizId}`, {
            method: 'POST',
            headers: {
          ...
          if (response.ok) {
            ...
            this.resetForm(); // Reset the form for the next question
          } else {
    ...
    mounted() {
      this.fetchChapterName(); // Fetch the chapter name when the component is mounted
    },
  };

  // Scoped styles for the component
  CreateNewQuestion.style = `
    <style scoped>
      #main {
